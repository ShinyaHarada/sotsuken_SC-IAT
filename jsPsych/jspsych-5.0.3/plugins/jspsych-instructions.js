...
jsPsych.plugins.instructions = (function () {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    trial.allow_backward =
      typeof trial.allow_backward === "undefined" ? true : trial.allow_backward;
    trial.allow_keys =
      typeof trial.allow_keys === "undefined" ? true : trial.allow_keys;
    trial.show_clickable_nav =
      typeof trial.show_clickable_nav === "undefined"
        ? false
        : trial.show_clickable_nav;
    ...
    function show_current_page() {
      ...
      if (trial.show_clickable_nav) {
        ...
        nav_html +=
          "<button id='jspsych-instructions-next' class='jspsych-btn'>次へ &gt;</button></div>";
    ...
    var after_response = function (info) {
      ...
      if (
        info.key === trial.key_backward ||
        info.key ===
          jsPsych.pluginAPI.convertKeyCharacterToKeyCode(trial.key_backward)
      ) {
        if (current_page !== 0 && trial.allow_backward) {
          back();
        }
      ...
      if (
        info.key === trial.key_forward ||
        info.key ===
          jsPsych.pluginAPI.convertKeyCharacterToKeyCode(trial.key_forward)
      ) {
        next();
      }
    };
